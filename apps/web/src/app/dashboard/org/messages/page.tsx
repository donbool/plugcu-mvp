...
export default function OrgMessagesPage() {
  ...
  const fetchThreads = async (orgId: string) => {
    const { data: threadsData } = await supabase
      .from('threads')
      .select(`
        *,
        brands(company_name, industry),
        events(title)
      `)
      .eq('org_id', orgId)
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!newMessage.trim() || !selectedThread || !org) return
      ...
      <DashboardLayout requiredRole="student_org">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            You need to create an organization profile to access messaging.
          </p>
          <Button onClick={() => window.location.href = '/dashboard/org/profile'}>
            ...
            {threads.length === 0 ? (
              <div className="text-center py-8 px-4">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Brands will contact you about your published events
                </p>
              </div>
            ) : (
                ...
                {threads.map((thread) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="font-semibold text-sm">{thread.brands.company_name}</h4>
                        {thread.brands.industry && (
                          <p className="text-xs text-gray-600">{thread.brands.industry}</p>
                        )}
                      </div>
          ...
          {selectedThread ? (
            <>
              <CardHeader className="border-b">
                <CardTitle className="text-lg">{selectedThread.brands.company_name}</CardTitle>
                <CardDescription>
                  {selectedThread.brands.industry}
